...
def build_query_frame(route: IntentRoute, state: AgentState | None = None) -> QueryFrame:
    ...
    if is_query_continuation(route) and not requested_attributes and state and state.query_frame:
        requested_attributes = state.query_frame.requested_attributes
    ...
    return QueryFrame(
        ...
        inherit_from_last_query_frame=is_query_continuation(route),
    )
